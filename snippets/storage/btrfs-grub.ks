...
part /boot/efi --ondisk=${KS_INSTALL_DISK} --fstype=efi   --size=2048   --label=efi  --fsoptions="umask=0077,shortname=winnt"
part /boot     --ondisk=${KS_INSTALL_DISK} --fstype=ext4  --size=2048   --label=boot --fsoptions="noatime,nodev,noexec,nosuid"
part btrfs.01  --ondisk=${KS_INSTALL_DISK} --fstype=btrfs --size=1 --grow            --fsoptions="compress=zstd:3,noatime,space_cache=v2"
